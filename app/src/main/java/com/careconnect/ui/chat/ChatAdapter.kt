...
/**
 * adapter della chat: i messaggi "miei" vanno a destra, quelli dell'altro a
 * sinistra (confronto mittenteId con l'uid corrente). Nella vista osservatore
 * del garante distingue anziano e volontario per nome e colore. mostraAscolto e
 * onAscolta servono al lato anziano
 */
class ChatAdapter(
    ...
    override fun onBindViewHolder(holder: MessaggioViewHolder, position: Int) {
        ...
        // la bolla del messaggio "riempita" (prugna, testo bianco) è ne  "i miei messaggi" e nella vista
        // osservatore del garante per quelli dell'anziano. (vanno a destra)
        // le altre sono bianche sono bianche con il testo prugna
        val riempita = if (mostraNomi) delAnziano else mio

        // nome del mittente: sono solo nella vista osservatore del garante/familiare
        if (mostraNomi) {
        ...
        // pulsante di lettura vocale che è presente solo lato anziano e solo sui messaggi
        // ricevuti
