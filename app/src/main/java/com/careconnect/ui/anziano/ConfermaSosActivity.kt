...
class ConfermaSosActivity : AppCompatActivity() {
    ...
    // riuso del ViewModel di T2: identica logica di invio SOS (un alert per
    // familiare -> push FCM). Lo istanzio con la stessa factory della Home.
    ...
    // il countdown vive solo mentre la schermata e' in primo piano (come in T2)
    ...
    override fun onStop() {
        super.onStop()
        // non fermare nulla se stiamo aprendo il dialer dopo la conferma
        if (!sosGiaInviato) {
            countdownJob?.cancel()
            tts.interrompi()
        }
    ...
    // fine countdown: riusa inviaSos() del ViewModel + apre il dialer sul 112
    private fun conferma() {
        ...
        try {
            ...
            // nessun dialer sul dispositivo: l'allarme ai familiari e' comunque partito
        }
        // la chiusura avviene in osservaEsitoSos(): se chiudessi qui, finish()
        // cancellerebbe il viewModelScope e con esso la scrittura su Firestore in corso
    }

    // aspetta il completamento dell'invio prima di chiudere (vedi nota in conferma())
